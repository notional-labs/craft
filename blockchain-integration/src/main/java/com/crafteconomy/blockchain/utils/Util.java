...
public class Util {
	...
	public static void log(String message) {
		if(CraftBlockchainPlugin.debugStatus()) logger.info(message);		
	}
	...
	public static void clickableRequestCraft(CommandSender sender, String command, String msgFormat) {
    ...
    public static void clickableWebsite(CommandSender sender, String URL, String fmt) {
        TextComponent message = new TextComponent(Util.color(fmt.replace("%url%", URL)));
            message.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, URL));
            message.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("§7§oSign this transcation with KEPLR")));
        sender.spigot().sendMessage(message);
    }
