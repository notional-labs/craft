...
export const getAllCollections = async (client: CosmWasmClient) => {
    // console.log(`Getting all collections`);

    const REDIS_KEY = `cache:all_collections`;
    const TTL = 30;  // TODO: Make this like 1 day+? would break new mints though.
    if (allowCache) { 
    ...
    // console.log(`All addresses ${collection_addresses}`);
    ...
    // only query pending_promises index 0 and 1
...
export const getTokensInCollection = async (client: CosmWasmClient, contract_address: string) => {
    // this is cached
    const collection_data = await getAllCollections(client);
