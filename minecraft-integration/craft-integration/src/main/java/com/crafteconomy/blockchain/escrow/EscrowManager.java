...
public class EscrowManager {
    ...
    public EscrowErrors depositUCraft(UUID uuid, long ucraft_amount) {
        // Get the most the user can withdraw from their chain ucraft wallet or what they want, which would be the minimum of the 2
    ...
    public long redeem(UUID uuid, long redeemAmountCraft) {
        ...
        // api.faucet();
        BlockchainRequest.depositToAddress(walletManager.getAddress(uuid), mostTheyCanRedeemUCraft);

        Player player = Bukkit.getPlayer(uuid);
            if(player != null) {
                player.sendMessage("You have redeemed " + (mostTheyCanRedeemUCraft*1_000_000) + CraftBlockchainPlugin.getInstance().getTokenDenom(false) + " from your escrow account -> wallet.");
                player.sendMessage("Your new escrow balance is: " + getCraftBalance(uuid));
            }  
    ...
    private static Consumer<UUID> depositEscrowLogic(UUID player_uuid, long ucraft_amount) {        
        Consumer<UUID> deposit = (uuid) -> {  
            ...
            if(player != null) {
                player.sendMessage("You have deposited " + ucraft_amount/1_000_000 + "craft into your escrow account.");
            }        
        };
        return deposit;
    }  
